...
export default async function(req) {
  try {
    ...
    // La productora solo puede editar operadores de su propia empresa
    if (role === 'productora') {
      if (targetRole !== 'operador') {
        return Response.json({ error: 'Solo podés editar operadores' }, { status: 403 });
      }
    ...
    // Construir payload solo con campos permitidos (sin tocar role ni company)
    const update = {};
