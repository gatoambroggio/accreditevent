// HTTP Digest auth (MD5) + helpers CGI para terminales Dahua.
// Implementación MD5 pura (Node no tiene MD5 nativo en crypto, y esto
// mantiene paridad con la versión Deno). Las terminales Dahua exponen
// CGI protegido con Digest (RFC 2617).
...
function rstr2binl(input) { const output = Array(input.length >> 2).fill(0); for (let i = 0; i < input.length * 8; i += 8) output[i >> 5] |= (input.charCodeAt(i / 8) & 0xff) << (i % 32); return output; }
function rstrMD5(s) { return binl2rstr(binlMD5(rstr2binl(s), s.length * 8)); }
function rstr2hex(input) { const hex = '0123456789abcdef'; let output = ''; for (let i = 0; i < input.length; i++) { const x = input.charCodeAt(i); output += hex.charAt((x >>> 4) & 0x0f) + hex.charAt(x & 0x0f); } return output; }
function str2rstrUTF8(input) { return Buffer.from(input, 'utf8').toString('binary'); }
...
function parseDigestHeader(header) {
  ...
  const re = /(\w+)\s*=\s*(?:"([^"]*)"|([^,]+))/g;
  let match;
  while ((match = re.exec(m)) !== null) params[match[1].toLowerCase()] = match[2] || match[3].trim();
  return params;
}
...
export async function dahuaRequest(device, path, opts = {}) {
  ...
  if (res.status !== 401) return { ok: res.ok, status: res.status, text: await res.text() };
...
export async function storeAccessUser(device, userId, userName) {
  return dahuaRequest(device, `/cgi-bin/accessControl.cgi?action=storeAccessUser&UserID=${enc(userId)}&UserName=${enc(userName)}&UserRole=0&UserType=0&Door=1&TimeSection=0000000100000000`);
}
export async function deleteAccessUser(device, userId) {
  return dahuaRequest(device, `/cgi-bin/accessControl.cgi?action=deleteAccessUser&UserID=${enc(userId)}`);
}
export async function storeFace(device, userId, imageBytes, mime = 'image/jpeg') {
  ...
  const tail = `\r\n--${boundary}--\r\n`;
  const body = Buffer.concat([Buffer.from(head, 'utf8'), Buffer.from(imageBytes), Buffer.from(tail, 'utf8')]);
  return dahuaRequest(device, `/cgi-bin/faceManager.cgi?action=storeFace&UserID=${enc(userId)}`, { method: 'POST', headers: { 'Content-Type': `multipart/form-data; boundary=${boundary}` }, body });
}
export async function openDoor(device, channel = 0) {
  return dahuaRequest(device, `/cgi-bin/accessControl.cgi?action=Control_Device&command=open&channel=${channel}&Type=Remote`);
}
export async function rebootDevice(device) {
  return dahuaRequest(device, `/cgi-bin/magicBox.cgi?action=reboot`);
}
export async function getAllUserIds(device) {
  const r = await dahuaRequest(device, `/cgi-bin/accessControl.cgi?action=getAllUserID`);
  const ids = r.text.split('\n').map((l) => l.split('=')[1]).filter(Boolean);
  return { ...r, ids };
}
export async function getDeviceStatus(device) {
  return dahuaRequest(device, `/cgi-bin/magicBox.cgi?action=getSystemInfo`);
}