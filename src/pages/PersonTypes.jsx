...
const FIELDS = [
  { name: 'label', label: 'Nombre', type: 'text', required: true, placeholder: 'Ej: Proveedor' },
  { name: 'value', label: 'Identificador (slug)', type: 'text', required: true, placeholder: 'provider', hint: 'Minúsculas, sin espacios ni caracteres especiales.' },
  { name: 'badge_prefix', label: 'Prefijo de credencial', type: 'text', required: true, placeholder: 'PR', hint: '2-3 letras para generar códigos de credencial.' },
];

export default function PersonTypes() {
  const { items, loading, create, update, remove } = useCrud('PersonType');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-wider text-emerald-600">Gestión de personas</p>
          <h1 className="mt-1 text-3xl font-extrabold tracking-tight text-slate-900">Tipos de persona</h1>
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        {loading ? (
          ...
          <p className="py-16 text-center text-sm text-slate-400">No hay tipos de persona configurados.</p>
        ) : (
      ...
      <EntityModal
        ...
        title={editing ? 'Editar tipo de persona' : 'Nuevo tipo de persona'}
        kicker={editing ? 'EDITAR TIPO' : 'CREAR TIPO'}
        fields={FIELDS}
        initialData={editing || {}}
