  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess({
        ...
        email: payload.email,
        tempPassword: !data.pending && useTempPassword && !data.passwordWarning ? tempPassword : '',
        inviteUrl: data.invite_url || '',
        warning: data.passwordWarning || (data.pending ? '' : '') || '',
      ...
      <div className="my-8 w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-200">
        ...
        {success ? (
          <div className="mt-5 space-y-3">
            ...
            {success.inviteUrl && (
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                <p className="text-xs font-semibold text-slate-600">Link de registro</p>
                <div className="mt-1 flex items-center gap-2">
                  <code className="flex-1 truncate rounded-lg bg-white px-3 py-2 text-xs text-slate-900 ring-1 ring-slate-200">{success.inviteUrl}</code>
                  <button onClick={() => navigator.clipboard.writeText(success.inviteUrl)} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-100">
                    <Copy className="h-3.5 w-3.5" /> Copiar
                  </button>
                </div>
                <p className="mt-1.5 text-[11px] text-slate-500">Compartilo con el operador para que complete el registro.</p>
              </div>
            )}
            {success.warning && (
