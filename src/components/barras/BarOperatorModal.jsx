...
import { Loader2, X, KeyRound, Wine, Ban, MailWarning, Info } from 'lucide-react';
...
export default function BarOperatorModal({ open, onClose, onSaved, bars, editing, template }) {
  ...
  const tplOk = !!(template && template.includes('{u}'));
  const derivedEmail = tplOk ? deriveEmail(template, username) : '';
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editing) {
      if (!tplOk) { setError('Falta configurar la plantilla de email de barras en Configuración.'); return; }
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/40 p-4 backdrop-blur-sm">
      <div className="my-8 w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl ring-1 ring-slate-200">
        ...
        <p className="mt-1.5 text-sm text-slate-500">
          {editing ? 'Modificá la barra asignada, el bloqueo o la clave del operador.' : 'El operador se loguea en la tablet sólo con usuario y contraseña.'}
        </p>
        ...
        {!editing && !tplOk && (
          <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
            ...
            <p>Falta configurar la <b>plantilla de email de barras</b> en Configuración. Sin eso no se pueden crear operadores. Pedí a un administrador que la configure (ej. <code className="rounded bg-amber-100 px-1">barras+{'{u}'}@ipx.com.ar</code>).</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-4">
          <label className="block">
            ...
            <input
              ...
              autoCorrect="off"
              placeholder="ej. B1"
              className="w-full rounded-lg border border-slate-200 px-3 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 disabled:bg-slate-50 disabled:text-slate-500"
            />
            {editing ? (
              <span className="mt-1 block text-[11px] text-slate-400">El usuario no se puede cambiar.</span>
            ) : tplOk && username ? (
              <span className="mt-1 block text-[11px] text-slate-500">Email de la cuenta: <span className="font-mono font-semibold text-emerald-700">{derivedEmail}</span></span>
            ) : null}
          </label>
          ...
          {editing ? (
            <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              {changePw && (
                <div className="mt-2 flex items-center gap-2">
                  <input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 6 caracteres" className="flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-mono outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20" />
            ...
            <label className="block">
              ...
              <div className="flex items-center gap-2">
                <input value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mínimo 6 caracteres" className="flex-1 rounded-lg border border-slate-200 px-3 py-2.5 text-sm font-mono outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20" />
          ...
          {!editing && tplOk && (
            <div className="flex items-start gap-2 rounded-lg bg-slate-50 p-3 text-[11px] text-slate-500">
              <Info className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              <p>La invitación llega al inbox compartido (<span className="font-mono">{derivedEmail || '…'}</span>). Vos la abrís una vez, completás el registro y el operador queda activo con la contraseña de arriba.</p>
            </div>
          )}
