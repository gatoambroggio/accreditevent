  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        <h4 className="text-base font-bold text-slate-900">{product.id ? 'Editar producto del catálogo' : 'Nuevo producto del catálogo'}</h4>
        <p className="mt-0.5 text-xs text-slate-500">Este es el menú base del evento. Al crear una barra se copia automáticamente; cada barra puede ajustar sus precios (ej. VIP).</p>
        <div className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-semibold text-slate-600">Precio base (ARS)</label>
