...
export default function AdminEmployeeImportModal({ open, onClose, onImport, companies, events }) {
  ...
  const handleImport = async () => {
    if (!companyName) { setError('Seleccioná una empresa para los empleados importados.'); return; }
    setImporting(true);
    setError('');
    try {
      await onImport(rows, companyName, eventId);
        ...
        <div className="flex items-start justify-between border-b border-slate-100 px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="mt-0.5 text-xl font-bold tracking-tight text-slate-900">Importar empleados desde Excel</h2>
            </div>
          </div>
        ...
        <div className="space-y-5 px-6 py-5">
          {result ? (
            <div className="flex flex-col items-center py-8 text-center">
              ...
              <p className="mt-1 text-sm text-slate-500">Los empleados ya están disponibles en Personal de Empresas.</p>
              ...
              {rows.length > 0 && (
                <>
                  <div className="flex items-center justify-between">
                    <p className="text-sm font-semibold text-slate-700">{rows.length} empleados a importar → {companyName || 'seleccioná empresa'}</p>
                    <button onClick={handleImport} disabled={importing || !companyName} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50">
